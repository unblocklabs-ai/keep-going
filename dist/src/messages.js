...
function isNoReplyAssistantText(value) {
...
function isSlackWrappedUserText(value) {
    ...
    return (SLACK_WRAPPED_USER_PREFIXES.some((prefix) => trimmed.startsWith(prefix)) ||
...
function extractMessageTexts(message) {
...
function collectToolCallNames(message) {
    ...
    if (Array.isArray(toolCalls)) {
        for (const entry of toolCalls) {
            ...
            const name = readToolCallName(entry);
            if (name) {
                names.add(name);
            }
        }
    }
    const functionCall = message.function_call && typeof message.function_call === "object"
...
function extractToolCallTexts(message) {
...
function stripAssistantNonHumanFacingLines(text) {
    return text
        .split("\n")
...
function normalizeHumanFacingAssistantText(text) {
...
export function normalizeHumanFacingUserText(text) {
...
export function extractSlackThreadHistoryMessages(text) {
    const trimmed = text.trim();
...
export function extractLastAssistantHumanFacingText(messages) {
    const transcript = normalizeTranscriptMessages(messages);
    for (let index = transcript.length - 1; index >= 0; index -= 1) {
        const entry = transcript[index];
        if (entry?.role !== "assistant") {
...
export function extractLastUserHumanFacingText(messages) {
    const transcript = normalizeTranscriptMessages(messages);
    for (let index = transcript.length - 1; index >= 0; index -= 1) {
        const entry = transcript[index];
        if (entry?.role !== "user") {
...
export function extractInitialSlackThreadHistoryMessages(messages) {
    const transcript = normalizeTranscriptMessages(messages);
...
export function lastAssistantHasSubagentSpawnToolCall(messages) {
