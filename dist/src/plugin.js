...
import { launchContinuation, resolveContinuationModelRoute, resolveContinuationSessionFile, } from "./launcher.js";
...
async function sendContinuationNoticeIfNeeded(runtime, context, reactionResult) {
    const { api, config, logger, deps } = runtime;
    const { candidate, route } = context;
    const shouldSend = config.continuationNotice.mode === "always" ||
    ...
    try {
        ...
        logger.step("sent continuation notice", {
            ...
            threadId: route.threadId,
            accountId: route.accountId,
            noticeReason: reactionResult.status === "sent"
                ? "mode-always"
